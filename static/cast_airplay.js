(() => {
  ...
  function refreshAction() {
    ...
    const disabled = !visible || count === 0;
  ...
  function ensureModal() {
    ...
    modal.addEventListener('click', event => {
      ...
      if (event.target.closest('[data-airplay-start]')) return openAirplayPlayer();
  ...
  function setSub(text) { const el = document.getElementById('castAirplaySub'); if (el) el.textContent = String(text || ''); }
  function setBody(html) { const el = document.getElementById('castAirplayBody'); if (el && el.innerHTML !== html) el.innerHTML = html; }
  function setError(message) {
    const el = document.getElementById('castAirplayError'); if (!el) return;
    const text = String(message || '').trim(); if (el.textContent !== text) el.textContent = text; el.classList.toggle('hidden', !text);
  }

  async function onActionClick(event) {
    event.preventDefault(); event.stopPropagation(); closeMapperMenu();
    const sel = selection();
    ...
    openModal(); setError(''); setSub(tr('Klargør valget…', 'Preparing selection…'));
    ...
    try {
      ...
      renderReady();
    } catch (error) { setError(error.message || error); setBody(''); }
  }

  function renderReady() {
    if (!lastSession) return;
    const count = Number(lastSession.item_count || 0);
    setSub(`${count} ${count === 1 ? tr('medie', 'item') : tr('medier', 'items')}`); setError('');
    if (platform() === 'ios') {
      const note = lastSession.contains_images
        ? tr('Video kan bruge Apples AirPlay-vælger. Billeder og blandede slideshows åbnes i fuld skærm og kan vises med Skærmdublering fra Kontrolcenter.', 'Video can use Apple’s AirPlay picker. Photos and mixed slideshows open fullscreen and can be shown with Screen Mirroring from Control Center.')
      ...
      return setBody(`<div class="cast-airplay-choice"><div class="cast-airplay-platform-icon"></div><div><strong>AirPlay</strong><p>${esc(note)}</p></div></div><div class="cast-airplay-actions"><button type="button" class="btn primary" data-airplay-start>${esc(tr('Åbn AirPlay', 'Open AirPlay'))}</button></div>`);
    }
    if (platform() === 'android') {
      if (lastSession.cast_configured) return setBody(`<div class="cast-airplay-choice"><div class="cast-airplay-platform-icon">▣</div><div><strong>Google Cast</strong><p>${esc(tr('Vælg Chromecast, Google TV eller Nest-skærm. FjordLens fortsætter selv slideshowet på modtageren.', 'Choose a Chromecast, Google TV or Nest display. FjordLens keeps the slideshow running on the receiver.'))}</p></div></div><div class="cast-airplay-actions"><button type="button" class="btn primary" data-cast-start>${esc(tr('Vælg Cast-enhed', 'Choose Cast device'))}</button></div>`);
      ...
      return setBody(`<div class="cast-airplay-note">${esc(tr('Google Cast er ikke konfigureret endnu. En administrator skal opsætte det først.', 'Google Cast is not configured yet. An administrator must set it up first.'))}</div>`);
    }
  ...
  async function saveCastSetup() {
    ...
    try {
      ...
      statusCache = status; lastSession.cast_receiver_app_id = status.cast_receiver_app_id || appId; lastSession.cast_configured = true;
      renderReady(); notify(tr('Google Cast er gemt.', 'Google Cast setup saved.'), 'ok');
  ...
  function loadCastSdk() {
    ...
    castSdkPromise = new Promise((resolve, reject) => {
      ...
      if (!document.querySelector('script[data-fjordlens-cast-sdk]')) {
        const script = document.createElement('script'); script.dataset.fjordlensCastSdk = '1'; script.async = true;
        script.src = 'https://www.gstatic.com/cv/js/sender/v1/cast_sender.js?loadCastFramework=1';
        script.onerror = () => reject(new Error(tr('Kunne ikke indlæse Google Cast SDK.', 'Could not load Google Cast SDK.'))); document.head.appendChild(script);
      }
  ...
  async function startGoogleCast() {
    ...
    try {
      ...
      await context.requestSession(); castSession = context.getCurrentSession();
      if (!castSession) throw new Error(tr('Cast-sessionen kunne ikke startes.', 'Could not start the Cast session.'));
      await new Promise(resolve => setTimeout(resolve, 650)); await sendCastLoad(true); renderCastControls();
    } catch (error) { const text = String(error?.message || error || ''); if (!/cancel/i.test(text)) setError(text); }
  }
  ...
  async function sendCastLoad(retry = false) {
    ...
    catch (error) { if (!retry) throw error; await new Promise(resolve => setTimeout(resolve, 900)); await sendCastMessage({type:'LOAD', session_url:lastSession.session_url}); }
  }
  async function sendCastCommand(type) { setError(''); try { await sendCastMessage({type}); } catch (error) { setError(error.message || error); } }
  function renderCastControls() {
    const count = Number(lastSession?.item_count || 0); setSub(tr('Caster nu', 'Casting now'));
  ...
  function openAirplayPlayer() { if (lastSession?.play_url) window.location.href = String(lastSession.play_url); }

  function init() {
    ensureAction(); refreshAction(); loadStatus().catch(() => {});
    ...
    const observer = new MutationObserver(() => refreshAction());
