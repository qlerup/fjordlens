...
class EditorBrowserTests(unittest.TestCase):
    ...
    def test_edit_reorder_pair_save_and_playback(self):
        ...
        <script src="/static/moments.js"></script><script src="/static/moment_music.js"></script><script src="/static/moment_player.js"></script><script src="/static/moment_text_editor.js"></script><script src="/static/moment_editor.js"></script>'''.replace('MOMENT_ID',str(moment['id']))
        errors = []
        with sync_playwright() as runtime:
            ...
            page = browser.new_page(viewport=dict(width=1440,height=1050), has_touch=True)
            ...
            page.locator('[data-field="eyebrow"]').fill('DANMARK')
            page.locator('[data-field="detail"]').fill('En sommerdag')
            sibling_before=page.locator('.slideshow-preview .cinema-detail').bounding_box()
            ...
            sibling_after=page.locator('.slideshow-preview .cinema-detail').bounding_box()
            self.assertAlmostEqual(sibling_before['x'],sibling_after['x'],delta=2)
            self.assertAlmostEqual(sibling_before['y'],sibling_after['y'],delta=2)
            ...
            self.assertEqual(page.locator('.slideshow-preview .cinema-separated').count(),1)
            page.get_by_role('button',name='Fortryd',exact=True).click()
            self.assertEqual(page.locator('.slideshow-preview .cinema-separated').count(),0)
            ...
            self.assertLess(saved[3]['text_elements']['heading']['x'], .5)
            self.assertLess(saved[3]['text_elements']['heading']['y'], .5)
            ...
            # A corner scales both dimensions equally; a side changes only width.
            snap=page.locator('.slideshow-snap-toggle input')
            snap.uncheck()
            def drag_handle(side,dx,dy):
                box=page.locator(f'.slideshow-preview [data-text-key=heading] [data-text-resize={side}]').bounding_box()
                page.mouse.move(box['x']+box['width']/2,box['y']+box['height']/2)
                page.mouse.down();page.mouse.move(box['x']+box['width']/2+dx,box['y']+box['height']/2+dy,steps=8);page.mouse.up()
            drag_handle('se',40,8)
            enlarged=heading.bounding_box()
            self.assertAlmostEqual(enlarged['width']/restored['width'],enlarged['height']/restored['height'],delta=.025)
            self.assertGreater(enlarged['width'],restored['width']+20)
            page.get_by_role('button',name='Fortryd',exact=True).click()
            drag_handle('e',40,0)
            widened=heading.bounding_box()
            self.assertAlmostEqual(widened['height'],restored['height'],delta=1)
            self.assertGreater(widened['width'],restored['width']+30)
            page.get_by_role('button',name='Fortryd',exact=True).click()
            snap.check()
            stage=page.locator('.slideshow-preview .cinema-slide').bounding_box()
            box=heading.bounding_box()
            page.mouse.move(box['x']+box['width']/2,box['y']+box['height']/2)
            page.mouse.down()
            page.mouse.move(stage['x']+stage['width']/2+3,stage['y']+stage['height']/2+3,steps=8)
            self.assertEqual(page.locator('.text-snap-guide').count(),2)
            page.mouse.up()
            centered=heading.bounding_box()
            self.assertAlmostEqual(centered['x']+centered['width']/2,stage['x']+stage['width']/2,delta=2)
            self.assertAlmostEqual(centered['y']+centered['height']/2,stage['y']+stage['height']/2,delta=2)
            page.get_by_role('button',name='Fortryd',exact=True).click()
            other=page.locator('.slideshow-preview .cinema-detail')
            a,b=heading.bounding_box(),other.bounding_box()
            page.mouse.move(b['x']+b['width']/2,b['y']+b['height']/2);page.mouse.down()
            page.mouse.move(a['x']+a['width']/2+.2,a['y']+a['height']+b['height']/2+.2,steps=8)
            self.assertEqual(page.locator('.text-snap-guide').count(),2)
            page.mouse.up()
            aligned=other.bounding_box()
            self.assertAlmostEqual(aligned['x']+aligned['width']/2,a['x']+a['width']/2,delta=2)
            self.assertAlmostEqual(aligned['y'],a['y']+a['height'],delta=2)
            page.get_by_role('button',name='Fortryd',exact=True).click()
            positioned = page.locator('.slideshow-preview [data-text-key=heading]')
            old_x = float(positioned.evaluate("e => parseFloat(e.style.left)"))
            positioned.focus()
            page.keyboard.press('ArrowRight')
            self.assertGreater(float(positioned.evaluate("e => parseFloat(e.style.left)")),old_x)
            page.get_by_role('button',name='Fortryd',exact=True).click()
            if os.environ.get('FJORDLENS_EDITOR_SCREENSHOT'):
            ...
            page.locator('#replayMoment:not([disabled])').wait_for()
            self.assertFalse(page.locator('#momentPlayerOverlay').is_visible())
            self.assertEqual(page.locator('video').count(),0)
            page.locator('#replayMoment').click()
            page.locator('#momentPlayerStage .cinema-slide').wait_for()
            self.assertEqual(page.locator('.moment-rotate-guide').count(),0)
            before_music=page.evaluate('state.momentPlayer.soundtrack.muted')
            ...
            self.assertEqual(page.evaluate('state.momentPlayer.soundtrack.muted'),before_music)
            page.get_by_role('button',name='Afslut fuld skærm',exact=True).click()
            page.wait_for_function('!document.fullscreenElement')
            self.assertEqual(page.evaluate('state.momentPlayer.soundtrack.muted'),before_music)
            # Emulate Safari's missing fullscreen/orientation lock: CSS rotation
            # must work independently, without autoplay or media during the hint.
            page.add_init_script("Element.prototype.requestFullscreen = () => Promise.reject(new Error('Unsupported')); Object.defineProperty(screen.orientation,'lock',{value:() => Promise.reject(new Error('Unsupported'))});")
            for width,height in ((390,844),(320,568)):
                ...
                self.assertFalse(page.locator('#momentPlayerOverlay').is_visible())
                page.locator('#replayMoment').click()
                page.locator('.moment-rotate-guide').wait_for()
                self.assertEqual(page.locator('#momentPlayerStage .cinema-slide').count(),0)
                self.assertTrue(page.evaluate('state.momentPlayer.preparing'))
                self.assertEqual(page.evaluate('state.momentPlayer.soundtrack.sources.size'),0)
                if screenshot and width == 390:
                    page.screenshot(path=str(Path(screenshot).with_name('fjordlens-rotate-hint.png')))
                page.wait_for_timeout(1000)
                self.assertEqual(page.locator('#momentPlayerStage .cinema-slide').count(),0)
                page.locator('.moment-rotate-guide').wait_for(state='detached')
                heading = page.locator('#momentPlayerStage .cinema-heading').last
                heading.wait_for()
                page.evaluate("() => { window.fullCalls=0; Element.prototype.requestFullscreen=()=>{window.fullCalls++;return Promise.reject(new Error('Unsupported'));}; }")
                before_music=page.evaluate('state.momentPlayer.soundtrack.muted')
                control=page.locator('#momentPlayerFullscreenBtn').bounding_box()
                page.touchscreen.tap(control['x']+control['width']/2,control['y']+control['height']/2)
                self.assertEqual(page.evaluate('window.fullCalls'),1)
                self.assertEqual(page.evaluate('state.momentPlayer.soundtrack.muted'),before_music)
                music_control=page.locator('#momentPlayerMusicBtn').bounding_box()
                page.touchscreen.tap(music_control['x']+music_control['width']/2,music_control['y']+music_control['height']/2)
                self.assertEqual(page.evaluate('state.momentPlayer.soundtrack.muted'),before_music)
                page.wait_for_timeout(850)
                page.locator('#momentPlayerMusicBtn').tap()
                self.assertNotEqual(page.evaluate('state.momentPlayer.soundtrack.muted'),before_music)
                self.assertEqual(page.evaluate('window.fullCalls'),1)
                page.emulate_media(reduced_motion='reduce')
                frame = page.locator('.moment-player-frame')
                self.assertIn('is-sideways',frame.get_attribute('class'))
                self.assertAlmostEqual(frame.evaluate('e => e.offsetWidth/e.offsetHeight'),16/9,delta=.01)
                self.assertLess(page.locator('#momentPlayerStage .cinema-type').last.evaluate('e=>e.offsetWidth'),frame.evaluate('e=>e.offsetWidth')*.3)
            ...
            page.wait_for_function("!document.querySelector('.moment-player-frame').classList.contains('is-sideways')")
            ...
            for _ in range(3):
                page.keyboard.press('ArrowRight')
            page.locator('#momentPlayerStage .cinema-separated').last.wait_for()
            page.set_viewport_size(dict(width=390,height=844))
            page.wait_for_function("document.querySelector('.moment-player-frame').classList.contains('is-sideways')")
            box = page.locator('#momentPlayerStage .cinema-separated').last.bounding_box()
            self.assertGreaterEqual(box['x'], -1)
            self.assertGreaterEqual(box['y'], -1)
            self.assertLessEqual(box['x']+box['width'],391)
            self.assertLessEqual(box['y']+box['height'],845)
            page.evaluate('_momentPlayerClose()')
            page.locator('#replayMoment').click()
            page.locator('.moment-rotate-guide').wait_for()
            page.get_by_role('button',name='Annuller',exact=True).click()
            page.wait_for_timeout(3200)
            self.assertIsNone(page.evaluate('state.momentPlayer'))
            self.assertFalse(page.locator('#momentPlayerOverlay').is_visible())
